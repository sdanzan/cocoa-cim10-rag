...
theory behind it, the architecture, and the rationale for each technical
choice. It's the counterpart to the READMEs (which say *how* to run things)
and to `IMPROVEMENTS.md` (which is a working log of what's still open) — this
one is meant to stand on its own for a reviewer.
...
benchmarked against alternatives yet (open item, see `IMPROVEMENTS.md`
item 13).
...
false-positive-heading case — see `IMPROVEMENTS.md` item 5.
...
`IMPROVEMENTS.md` is the working log of what's been tried, measured, and
decided, and the open backlog of what's next (chunking rework, hierarchy
metadata, hybrid search, prompt few-shotting, model benchmarking, query
analytics). `IMPROVEMENTS-PHASE1.md`, `-E18.md` and `-DDD.md` are the detailed
records behind specific entries in that log.
